...
thresholdVal = 200
maxVal = 255
retValue,imgThreshold = cv2.threshold(blur,thresholdVal,maxVal,cv2.THRESH_BINARY) # THRESH_TOZERO_INV
# imgThreshold_rgb = cv2.cvtColor(imgThreshold,cv2.COLOR_GRAY2RGB) # return value is not used
cv2.imshow("thershold",imgThreshold)

blur = cv2.blur(imgThreshold, (3,3))
...
retValue,imgThreshold = cv2.threshold(blur,thresholdVal,maxVal,cv2.THRESH_BINARY) # THRESH_TOZERO_INV
# imgThreshold_rgb = cv2.cvtColor(imgThreshold,cv2.COLOR_GRAY2RGB) # return value is not used
# cv2.imshow("thershold2",imgThreshold)
...
retValue,imgThreshold = cv2.threshold(blur,thresholdVal,maxVal,cv2.THRESH_BINARY) # THRESH_TOZERO_INV
# imgThreshold_rgb = cv2.cvtColor(imgThreshold,cv2.COLOR_GRAY2RGB) # return value is not used
cv2.imshow("thershold3",imgThreshold)
